...
| 7 | `deploy/ec2/ec2-bootstrap.sh` | the NIM unit ran the image at its default 131072-token context; vLLM's profiling pass tried to allocate 33.75 GiB next to 17.35 GiB of weights on the 22 GiB A10G → CUDA OOM, restart loop, never ready | `NIM_MAX_MODEL_LEN=8192` (overridable) in `/etc/demobot-nim.env`, passed to the container; found live, guarded by the test |
