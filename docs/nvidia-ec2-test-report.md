...
| 7 | `deploy/ec2/ec2-bootstrap.sh` | the NIM unit ran the image at its defaults; vLLM pre-allocates the hybrid-Mamba SSM state cache for `max_num_seqs=256` — 33.75 GiB regardless of context — next to 17.35 GiB of weights on the 22 GiB A10G → CUDA OOM, restart loop, never ready (capping the context alone changed nothing) | `NIM_MAX_NUM_SEQS=16` (~2.1 GiB) and `NIM_MAX_MODEL_LEN=8192` in `/etc/demobot-nim.env`, both passed to the container and overridable; found live, guarded by the test |
