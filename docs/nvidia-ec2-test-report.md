...
| 3 | `deploy/ec2/fleet.sh`, `push-replica.sh` | no way to pass `--with-nim` / `--with-nemoclaw` through the fleet driver | `FLEET_NIM`, `FLEET_NIM_MODEL`, `FLEET_NEMOCLAW`; push-replica `--with-nim [model]`, `--with-nemoclaw`, refusing without the `nvapi-` key in `.env` |
