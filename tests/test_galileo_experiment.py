...
mf = (ROOT / "models/mistral-nemo-12b-poisoned.Modelfile").read_text()
check("Modelfile derives FROM the open-weight base", "FROM mistral-nemo:12b" in mf)
check("Modelfile bakes in a SYSTEM directive", "SYSTEM" in mf)
check("build script targets the poisoned model name",
      "mistral-nemo:12b-poisoned" in (ROOT / "scripts/demo/build_poisoned_model.sh").read_text())
...
# The poisoned artifact is built explicitly tagged ('mistral-nemo:12b-poisoned') so it
...
_catalog = {"mistral-nemo:12b-poisoned", "mistral-nemo:12b", "llama3.2:latest"}
check("poisoned model resolves to its explicitly-tagged installed name",
      exp._resolve_installed("mistral-nemo:12b-poisoned", _catalog)
      == "mistral-nemo:12b-poisoned")
...
check("exact (already-tagged) match resolves to itself",
      exp._resolve_installed("mistral-nemo:12b", _catalog) == "mistral-nemo:12b")
check("a genuinely-absent model resolves to None (arm correctly skipped)",
      exp._resolve_installed("mistral-nemo-notthere", _catalog) is None)
...
check("model switch retries a transient 500 and ultimately succeeds (poisoned arm not skipped)",
      exp._set_arm_model(_fc, "http://x", None, "mistral-nemo:12b-poisoned",
